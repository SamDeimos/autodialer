...
if (empty($_GET['id'])) {
    ...
    $querycampana = mysqli_query($conAutodialer, $sqlcampana);
    $resultcampana = mysqli_fetch_array($querycampana);
...
WHERE campana = '$id'";
    ...
    $result = mysqli_query($conAstetisk, $sql_query);			// Resultado
    $fields_cnt = mysqli_num_fields($result);	// cantidad de campos
    $schema_insert = '';
    for ($i = 0; $i < $fields_cnt; ++$i) {
        $l = $csv_enclosed.str_replace($csv_enclosed, $csv_escaped.$csv_enclosed,
            stripslashes(mysqli_fetch_field_direct($result, $i))).$csv_enclosed;	// Nombres de los campos
        $schema_insert .= $l;
